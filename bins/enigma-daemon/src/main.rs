...
async fn main() {
    ...
    let path = args.get(1).cloned().unwrap_or_else(|| "config.json".to_string());
    let content = fs::read_to_string(&path).unwrap_or_default();
    let parsed: DaemonConfigWrapper = serde_json::from_str(&content).unwrap_or(DaemonConfigWrapper { core: None });
    let config = parsed.core.unwrap_or_default();
    let registry = Arc::new(InMemoryRegistry::new());
    let relay = Arc::new(InMemoryRelay::new());
    let transport = Arc::new(MockTransport::new());
    let core = Core::init(config, Policy::default(), Arc::new(DaemonKey), registry, relay, transport)
        .await
        .expect("daemon init");
